...
public class Block
{
	...
	*/
    ...
    {
    	if (!gr.isValid(loc)) throw new IllegalArgumentException(); 
    	
    	//if already occupied...take it out and put in a new one
    	
    	Block prevBlock = gr.get(loc); 
    	
        if (prevBlock != null)
            prevBlock.removeSelfFromGrid(); 
    ...
    public void moveTo(Location newLocation)
    {
    	...
    	if (newLocation.equals(location)) return; //do nothing
    	
         MyBoundedGrid<Block> currGrid = grid; //we can access a copy of the Grid object
         
         //if the new location isn't empty...
         
         Block previousOccupant = grid.get(newLocation); 
         if (previousOccupant != null)
             previousOccupant.removeSelfFromGrid(); 
        
         //instructs the block to change its location
         
         removeSelfFromGrid();
         putSelfInGrid(currGrid, newLocation);
