...
public class MyArrayList<E> implements MyList<E>, MyListIterator<E> //class definition
{
	...
	public void add(int index, E obj) 
	{
		...
		if (myArraySize == myArray.length) // checks if the array is already full
			doubleSize(); //doubles the capacity in preparation for the new elements
		
		else //if the array isn't full
			for (int i = myArraySize-1; i >=index; i--) //traverse from the end
			{
				myArray[i+1]=myArray[i]; // push each value one to the right
			}
		
		myArray[index]=obj; //set the actual index to the object
